//! Native test suite for the Stylus `PerpEngine` (extracted from lib.rs).
    ...
    const WITHDRAWAL_PREVIEW_DIFF_FIXTURE: &str =
        include_str!("../../test/fixtures/withdrawal_preview_differential.json");
    ...
    // The withdrawal preview exists TWICE — here and in the Solidity reference — written to mirror
    // each other. Each side has its own tests; this is the only thing that proves they AGREE.
    // The fixture is generated by test/differential/WithdrawalPreviewDifferential.t.sol driving the
    // real Solidity PerpPair, and replayed here bit-exactly.
    #[test]
    fn withdrawal_preview_differential() {
        let root: Value = serde_json::from_str(WITHDRAWAL_PREVIEW_DIFF_FIXTURE).expect("withdrawal fixture");
        ...
        for c in cases {
            ...
            let g = |k: &str| u256s(c[k].as_str().unwrap_or_else(|| panic!("{label}: missing {k}")));
            let b = |k: &str| c[k].as_bool().unwrap_or_else(|| panic!("{label}: missing {k}"));

            let block_ts = g("blockTs").to::<u64>();
            let vm = TestVM::new();
            vm.set_block_timestamp(block_ts);
            let mut e = PerpEngine::from(&vm);
            seed_trade_engine(&mut e);
            e.last_operation_timestamp.set(U64::from(block_ts));

            e.global_liquidity_stable.set(g("poolStable"));
            e.global_liquidity_asset.set(g("poolAsset"));

            let user = addr(0xBE);
            {
                let mut up = e.user_virtual_trader_position.setter(user);
                up.balance_stable.set(g("balanceStable"));
                up.balance_asset.set(g("balanceAsset"));
                up.debt_stable.set(g("debtStable"));
                up.debt_asset.set(g("debtAsset"));
                up.funding_fee.set(g("fundingFee"));
                up.funding_fee_sign.set(b("fundingFeeSign"));
            }

            // Scaled-identity snapshot and epoch matrix, exactly as the generator seeds them, so LP
            // recovery returns the seeded balances and the comparison pins the PREVIEW.
            let scale = e.liquidity_m_decimals.get();
            let (lp_stable, lp_asset) = (g("lpInitialStable"), g("lpInitialAsset"));
            {
                let mut ep = e.liquidity_epochs.setter(U256::ZERO);
                ep.liquidity_m00.set(scale);
                ep.liquidity_m11.set(scale);
                if lp_stable != U256::ZERO || lp_asset != U256::ZERO {
                    ep.active_lp_count.set(U256::from(1u64));
                }
            }
            {
                ...
                lp.initial_stable_balance.set(lp_stable);
                lp.initial_asset_balance.set(lp_asset);
                lp.debt_stable.set(g("lpDebtStable"));
                lp.debt_asset.set(g("lpDebtAsset"));
            }
            e.liquidity_position_epoch.setter(user).set(U256::ZERO);

            e.dx0.set(g("dx0"));
            e.dy0.set(g("dy0"));
            e.last_curve_update.set(U64::from(g("lastCurveUpdate").to::<u64>()));
            e.last_trade_direction.set(b("lastTradeDirection"));
            e.last_validated_price.set(g("lastValidatedPrice"));

            let (pnl, pnl_sign, margin_safe) = e
                .withdrawal_check_data(user, g("price"), g("hypotheticalCollateral"))
                .unwrap_or_else(|_| panic!("{label}: preview reverted"));

            assert_eq!(pnl, g("pnl"), "{label}: pnl");
            assert_eq!(pnl_sign, b("pnlSign"), "{label}: pnlSign");
            assert_eq!(margin_safe, b("marginSafe"), "{label}: marginSafe");
        }
    }
