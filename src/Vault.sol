...
contract Vault is AccessControl, ReentrancyGuardTransient, ERC2771Context, Pausable {
    ...
    uint256 public immutable minCollateralMovement;
    ...
    function removeCollateral(uint256 amount, bytes memory unverifiedReport) public nonReentrant whenNotPaused {
        ...
        for (uint256 i; i < stableCoins.length; i++) {
            if (removedCollateral[i] > 0) {
                (bool success,) = address(stableCoins[i].stableCoin)
                    .call(abi.encodeWithSelector(IERC20.transfer.selector, user, removedCollateral[i]));
                if (!success) {
                    IERC20(address(stableCoins[i].stableCoin)).approve(lostAndFound, removedCollateral[i]);
                    ILostAndFound(lostAndFound)
                        .depositLostFunds(user, address(stableCoins[i].stableCoin), removedCollateral[i]);
    ...
    function _updateUserRatios(uint256 amount, address user, uint256[] memory removedCollateral) private {
        uint256 normFact = 0;
        for (uint256 i = 0; i < stableCoins.length; i++) {
            // Recalculate removed collateral using vault ratios
            removedCollateral[i] = totalCollateralRatio[stableCoins[i].stableCoin] * amount / ratioDecimals;
            uint256 oldStableCoinAmount =
                userCollateralRatio[user][stableCoins[i].stableCoin] * userCollateral[user] / ratioDecimals;
            if (oldStableCoinAmount > removedCollateral[i] && userCollateral[user] > amount) {
                userCollateralRatio[user][stableCoins[i].stableCoin] =
                    (oldStableCoinAmount - removedCollateral[i]) * ratioDecimals / (userCollateral[user] - amount);
            } else {
                userCollateralRatio[user][stableCoins[i].stableCoin] = 0;
            }
        ...
        if (normFact != ratioDecimals && normFact != 0) {
            for (uint256 i = 0; i < stableCoins.length; i++) {
                userCollateralRatio[user][stableCoins[i].stableCoin] =
                    userCollateralRatio[user][stableCoins[i].stableCoin] * ratioDecimals / normFact;
            }
        }
    }
    ...
    function initializeParameters(
        address _perpPairAddress,
        address _lostAndFoundAddress
    )
        external
        ...
        onlyRole(MOD_ROLE)
    {
        bytes32 paramHash = keccak256(
            abi.encodePacked(
                stableCoin, depositRatioThreshold, withdrawalRatioThreshold, stableDecimals, newTimeLockDuration
            )
        );
        ...
        emit AddingStableCoin(
            0, stableCoin, depositRatioThreshold, withdrawalRatioThreshold, stableDecimals, newTimeLockDuration
        );
    }
        ...
        onlyRole(MOD_ROLE)
    {
        ...
        emit AddingStableCoin(
            addStableTimeLock,
            stableCoin,
            depositRatioThreshold,
            withdrawalRatioThreshold,
            stableDecimals,
            newTimeLockDuration
        );
    }
