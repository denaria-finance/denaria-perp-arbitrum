...
library UtilMath {
        ...
        returns (uint256 marginRatio)
    {
        ...
        (
            ...
            uint256 fundingFee,
            bool fundingFeeSign,,
        ) = getUserVirtualTraderPosition(perpPair, user);
        ...
        if (lastOperationTimestamp != block.timestamp) {
            (uint256 newFundingRate, bool newFundingRateSign) =
                getComputeFundingRate(perpPair, price, lastOperationTimestamp);
            (fundingRate, fundingRateSign) = signedSum(fundingRate, fundingRateSign, newFundingRate, newFundingRateSign);
        }
        (uint256 localFundingFee, bool localFundingFeeSign) =
            getComputeFundingFee(perpPair, user, fundingRate, fundingRateSign);
        ...
        returns (uint256 marginRatio)
    {
        ...
        marginRatio = totColl * MMRDecimals / (positionValue);
    }
        ...
        returns (uint256 pnl, bool pnlSign)
    {
        ...
        if (diffAsset > 1e13 * oracleDecimals / price) {
            if (useSpotPrice) {
                ...
                shortReturn = computeShortReturn(
                    perpPair,
                    diffAsset,
                    price,
                    oracleDecimals,
        ...
        returns (uint256 slippage)
    {
        ...
        slippage = SignedMath.abs(SafeCast.toInt256(actualPrice) - SafeCast.toInt256(spotPrice)) * slippageDecimals
            / spotPrice;
    }
        ...
        )
    {
        ...
        // ---- Curve computation ----
        {
            (uint256 shortA, uint256 shortB, uint256 longA, uint256 longB) = getLongCurveParameters(p.perpPair);

            if (p.direction) {
                s.tradeReturn = computeLongReturn(
                    p.perpPair,
                    p.size,
                    p.price,
                    1e8,
                ...
                s.tradeReturn = computeShortReturn(
                    p.perpPair,
                    p.size,
                    p.price,
                    1e8,
        ...
        s.initialGuess = p.direction ? s.globalLiquidityAsset - s.tradeReturn : s.globalLiquidityStable - s.tradeReturn;
        ...
        // ---- Funding fee update ----
        {
            ...
            (uint256 localFee, bool localFeeSign) = getComputeFundingFee(p.perpPair, p.user, newRate, newSign);
    ...
    function getUserVirtualTraderPosition(
        address perpPair,
        address user
    )
        private
    ...
    function getPrice(address perpPair) private view returns (uint256 price) {
    ...
    function getMMR(address perpPair) private view returns (uint256 MMR) {
    ...
    function getLpLiquidityBalance(
        address perpPair,
        address user
    )
        private
    ...
    ///@notice Reads the collateral of a user. The Stylus engine does not expose
    /// `getCollateral(user)` (per-user collateral lives in the Vault), so the Vault is
    /// resolved from `ReadParameters()[0]` and read directly. Value-identical to the legacy
    /// `PerpPair.getCollateral`, which was itself a proxy to `IVault(vault).userCollateral(user)`
    /// (see `internalPerpLogic.sol`) — so this path works against BOTH the legacy Solidity
    /// pair and the Stylus engine.
    ///@param perpPair Address of the perpPair contract.
    ///@param user Address of the user.
    ...
    ///@notice Staticcall method to get the optional CurveMath adapter from the perpPair contract.
    ///@param perpPair Address of the perpPair contract.
    ///@return adapter CurveMath adapter address, or address(0) when the perpPair uses Solidity CurveMath.
    function getCurveMathAdapter(address perpPair) private view returns (address adapter) {
        (bool success, bytes memory data) = perpPair.staticcall(abi.encodeWithSignature("curveMathAdapter()"));
    ...
    function getLongCurveParameters(address perpPair)
        private
        view
        returns (uint256 shortA, uint256 shortB, uint256 longA, uint256 longB)
    {
    ...
    function getComputeFundingFee(
        address perpPair,
        address user,
        uint256 newFundingRate,
        bool newFundingRateSign
    )
        private
        view
    ...
    function getComputeFundingRate(
        address perpPair,
        uint256 price,
        uint256 timestamp
    )
        private
        view
        returns (uint256 localFundingRate, bool localFundingRateSign)
    {
    ...
    function calcPnLNoExit(address user, uint256 price, address perpPair) public view returns (uint256, bool) {
        ...
        (uint256 localFundingFee, bool localFundingFeeSign) =
            getComputeFundingFee(perpPair, user, newFundingRate, newFundingRateSign);
        (localFundingFee, localFundingFeeSign) =
            signedSum(fundingFee, fundingFeeSign, localFundingFee, localFundingFeeSign);
