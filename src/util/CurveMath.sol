...
library CurveMath {
        ...
        returns (uint256 c, bool cSign)
    {
        ...
        uint256 c2 =
            xSquare * priceScaled / 1e18 * priceScaled / 1e18 * bPlusScaled / 1e18 * (bPlusScaled + 2 * 1e18) / 1e18;
        ...
        uint256 nom = xSquare * longCurveParameterA / curveParameterDecimals * priceScaled / 1e18 * priceScaled / 1e18
            * diffAsset;
        ...
        returns (uint256 d)
    {
        ...
        d = xSquare * scaledPrice / 1e18 * scaledPrice / 1e18 * scaledPrice / 1e18 * initialAsset / 1e18 * initialAsset
            / 1e18 * bPlusScaled / 1e18 * bPlusScaled / 1e18;
    }
    ...
    function computeInverseKShort(uint256 y0, uint256 px0) public pure returns (uint256 kShortInv, bool kShortInvSign) {
        ...
        returns (uint256 b, bool bSign)
    {
        ...
        uint256 term3 = 2 * y * (bParam + curveParameterDecimals) / curveParameterDecimals * y / y0 * 1e18 / y0
            * oracleDecimals / p; //Sign is -
        ...
        returns (uint256 aPrime)
    {
        ...
        return
            (A * scaledP / 1e18 * scaledP / 1e18 * x0 / 1e18 * x0 / 1e18 * x0 / 1e18 * x0) / (x0 * scaledP / 1e18 + y0);
    }
        ...
        returns (uint256, bool)
    {
        ...
        uint256 term2 = x0 * x0 / 1e18 * (bParam + curveParameterDecimals) / curveParameterDecimals
            * (bParam + curveParameterDecimals) / curveParameterDecimals * p / oracleDecimals * p / oracleDecimals * x
            / x0 * 1e18 / x0;
        ...
        returns (uint256 outputSize)
    {
        ...
        uint256 d = computeLongD(xSquare, prc, globAss, oracleDec, longCurveParameterB, curveParameterDecimals);
